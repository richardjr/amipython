...
- Docker (optional, for cross-compiling to Amiga)
...
This installs the `amipython` CLI command and the `pytest` dev dependency.
...
This uses the Phase 1 language subset: typed variables, functions with annotated parameters, `print()`, and `for i in range()` loops.
...
This produces `hello.c` and copies `amipython.h` alongside it. The generated C89 looks like:
...
- `int` maps to `LONG` (32-bit), because vbcc's `int` is 16-bit on 68k
- All variable declarations are at the top of each scope (C89 requirement)
- `print("Hello,", name)` becomes separate `amipython_print_*()` calls with space separators and a trailing newline
- `amipython.h` provides dual-target I/O: on Amiga it uses dos.library `Output()`/`Write()` (vamos-compatible); on host it uses `printf()`

### Compile and run on host with gcc

You can compile and run the generated C directly on your Linux/macOS machine for fast iteration — no emulator needed:
...
The `amipython.h` header has `#ifdef AMIGA` guards so the same generated code compiles with both host gcc and the Amiga cross-compiler.

### Cross-compile to Amiga with Docker

To produce a native 68k Amiga executable, pull the vbcc Docker image and use `amipython build`:

```bash
# Pull the cross-compiler image (one-time, ~1 GB)
docker pull walkero/docker4amigavbcc:latest-m68k

# Transpile + cross-compile in one step
amipython build hello.py
```

This produces a `hello` binary in Amiga Hunk format. You can verify the magic bytes:

```bash
xxd hello | head -1
# Should start with: 0000 03f3 (Amiga Hunk magic)
```

To run it, use [vamos](https://github.com/cnvogelg/amitools) (no Kickstart ROM needed):

```bash
# Install amitools from git main (the pip release has version compatibility issues)
pip install "git+https://github.com/cnvogelg/amitools.git@main#egg=amitools"

vamos hello
```

The output binary is a standard AmigaOS CLI executable in Amiga Hunk format. On a real Amiga or full emulator (Amiberry with Kickstart ROM and Workbench installed), you'd run it from a Shell. For Phase 1 CLI programs, vamos is the recommended test environment since it doesn't require ROMs or a full system.

### Running the tests

```bash
# Run all tests (excluding Docker cross-compilation)
pytest

# Run only the Docker cross-compilation test (requires the vbcc image)
pytest -m docker
...
- **Validator tests** — checks that supported syntax is accepted and unsupported features (classes, imports, decorators, comprehensions, etc.) are rejected with clear errors
- **Type checker tests** — literal inference, type annotations, arithmetic promotion rules, function signatures, type consistency enforcement
- **Emitter tests** — verifies generated C for each construct: variables, arithmetic, print, control flow, functions, boolean operators
- **Golden file tests** — transpiles fixture `.py` files and compares output against expected `.c` files
- **Host compilation tests** — transpiles Python, compiles with `gcc -std=c89 -pedantic -Wall -Werror`, runs the binary, and checks stdout for correct output (including Python-specific semantics like floor division rounding toward -infinity and modulo matching the divisor's sign)
- **Docker cross-compilation test** (`pytest -m docker`) — cross-compiles with vbcc, verifies the output is a valid Amiga Hunk binary (magic bytes `0x000003F3`)
...
The language is not full Python. It is a restricted subset sufficient for game logic, tailored for game development:

- Variables, functions, if/else, while/for loops
- Basic types: int, float, bool, str
- Collections: list (typed, homogeneous), tuple
- Classes with type-annotated fields (maps to C structs)
- Implicit static typing (a variable holds one type throughout its lifetime)
- No dynamic features: no eval, getattr, metaclasses, decorators, generators, closures
- No garbage collection — static allocation and arena allocators
- Game-specific builtins map directly to engine API calls

## Language Design: Blitz Basic → amipython
...
| `palette` | Colour management (OCS/ECS/AGA) | `RGB`, `PalRGB`, `AGAPalRGB` |
...
## Side-by-Side Examples: Blitz Basic → amipython
...
**amipython**:
```python
from amiga import Display, Bitmap, palette
...
| Blitz | amipython | Notes |
|---|---|---|
| `RGB reg,r,g,b` | `palette.set(reg, r, g, b)` | OCS/ECS 12-bit |
| `AGAPalRGB pal,reg,r,g,b` | `palette.aga(reg, r, g, b)` | AGA 24-bit |
...
### Phase 1 — Transpiler Core (done)
Parse the Python subset, emit C89. Get `hello world` through the full pipeline: `.py` → `.c` → Amiga binary. Supports `int`, `float`, `bool`, `str`, functions, `if`/`elif`/`else`, `while`, `for i in range()`, `print()`, `global`, `break`/`continue`/`pass`, arithmetic with Python semantics (floor division, modulo, power). See [Getting Started](#getting-started) above.

### Phase 2 — Display + Drawing
`Display`, `Bitmap`, drawing primitives, palette. Enough to reproduce the simple display examples.

### Phase 3 — Game Loop + Sprites/Bobs
`run()`, double buffering, `Shape`, `Sprite`, blitting. Enough for bouncing balls demo.

### Phase 4 — Input + Scrolling
`joy`, `key`, `mouse`, `Tilemap`. Enough for a side-scroller.

### Phase 5 — Copper, Collision, Dual Playfield, Audio
Per-scanline copper effects, hardware collision, dual playfield, Paula sound.

## Cross-Compilation Toolchain

### Compiler Options

- **vbcc** — Lightweight C89 compiler producing the smallest binaries. C only. Docker: `walkero/docker4amigavbcc:latest-m68k`.
- **Bebbo's amiga-gcc** — GCC fork for m68k-amigaos. C and C++. Fastest code. Docker: `amigadev/crosstools:m68k-amigaos`.
...
- **Amiberry** — Full Amiga emulator. Headless (`-G`), directory mounting (`-m`), IPC socket (`/tmp/amiberry.sock`), auto-launch via startup-sequence.
- **vamos** — Headless AmigaOS API emulator from [amitools](https://github.com/cnvogelg/amitools). No Kickstart ROM required. CLI binaries only.
...
[ACE (Amiga C Engine)](https://github.com/AmigaPorts/ACE) is an actively maintained C game framework that handles display management, Blitter, Copper, tilemaps, input, audio, and chip/fast RAM allocation.
