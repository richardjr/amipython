...
/* FONT6_BEGIN — generated by scripts/gen_font6.py, do not edit */
...
static const UBYTE s_font6x8[][8] = {
    ...
    /* 0x21 '!' */ {0x20,0x20,0x20,0x20,0x00,0x00,0x20,0x00},
    /* 0x22 '"' */ {0x50,0x50,0x00,0x00,0x00,0x00,0x00,0x00},
    /* 0x23 '#' */ {0x50,0xF8,0x50,0x50,0xF8,0x50,0x00,0x00},
    /* 0x24 '$' */ {0x20,0x78,0x80,0x70,0x08,0xF0,0x20,0x00},
    /* 0x25 '%' */ {0xC0,0xC8,0x10,0x20,0x40,0x98,0x18,0x00},
    /* 0x26 '&' */ {0x60,0x90,0x60,0x68,0x90,0x90,0x68,0x00},
    ...
    /* 0x3A ':' */ {0x00,0x60,0x60,0x00,0x60,0x60,0x00,0x00},
    /* 0x3B ';' */ {0x00,0x60,0x60,0x00,0x60,0x20,0x40,0x00},
    ...
    /* 0x57 'W' */ {0x88,0x88,0x88,0xA8,0xA8,0xA8,0x50,0x00},
...
/* Glyph cell width for a bitmap's current font (0 = never set = 8). */
static LONG _fontWidth(const AmipyBitmap *bm) {
    return bm->fontWidth == 6 ? 6 : 8;
}
...
void amipython_bitmap_print_at(AmipyBitmap *bm, LONG x, LONG y, const char *text, LONG color) {
    ...
    while (*text) {
        ...
        if (ch >= 0x20 && ch <= 0x5A) {
            glyph = (gw == 6) ? s_font6x8[ch - 0x20] : s_font8x8[ch - 0x20];
        } else {
...
static LONG _pieces_total_width(const AmipyBitmap *bm, const char **strs, LONG n) {
    LONG total = 0;
    LONG gw = _fontWidth(bm);
    LONG i;
    const char *p;
...
void amipython_bitmap_print_centered(AmipyBitmap *bm, LONG y, const char *text, LONG color) {
    ...
    strs[0] = text;
    {
        LONG w = _pieces_total_width(bm, strs, 1);
...
void amipython_bitmap_print_centered_multi(AmipyBitmap *bm, LONG y, LONG color, LONG n, ...) {
    ...
    va_end(ap);
    w = _pieces_total_width(bm, strs, n);
    x = ((LONG)bm->width - w) / 2;
...
/* Every storage call is bracketed by these two. Besides silencing DOS
 * requesters they hand the OS back for the duration of the file I/O when
 * ACE has taken it over (after display.show): dos.library calls with the
 * system "unused" never return — the title-screen save probe of a game
 * hung the Amiga this way. systemUse()/systemUnuse() are ACE's sanctioned
 * way to do mid-game file I/O; the screen may blink for a frame. */
static UBYTE s_bDosReenabledSystem = 0;
...
static void _restore_dos_requesters(APTR old) {
    ...
    pr->pr_WindowPtr = old;
    if (s_bDosReenabledSystem) {
        systemUnuse();
        s_bDosReenabledSystem = 0;
    }
}
...
BOOL amipython_storage_exists(const char *name) {
    ...
    if (f) Close(f);                 /* Close is a DOS call too — before the restore */
